...
%		and generates a truly random seed $s$ and send it to \garbler.
...
\begin{itemize}[leftmargin=*]
	...
	\item To subscribe computation $C$, subscriber sends a subscription request
		...
		%and requests an output mask $r$ for computation $C$ from \garbler.
...
%		\garbler checks if it has received wire labels, $w_0$ and $w_1$ for a bit
		%		$b$, from all publishers in $P_C$. If so, it generates garbled circuit,
		%		$GC$ for $XOR \circ C$, composition of $XOR$ masking function and $C$
		%		and sends garbled circuit to \broker. $XOR$ adds a pseudorandom mask
		%		$r$ to the output of $C$.
