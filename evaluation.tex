...
the distance between nodes and their reported signal strength measures).  $eta$
can be derived from the following formula: $RSS = -10 \cdot eta \cdot
log_{10}(distance)$.  We will estimate $eta$ by finding a linear model for the
$RSS$-distances pairs obtained from the nodes.
...
a linear combination of the distance.  To estimate the parameters of the one
dimensional linear model ($y = ax + b$) we use the ordinary least squares
technique, which gives us the following closed-form formula:
...
computer.  The Publishers will publish the distances to their peers as an
initial step, allowing the \broker to store these values for some time avoiding
the constant retransmission of these constants.  After this, the Publishers
will be sending values at the same rate of the nodes in order to obtain the
same performance results we would get by using live data from the actual
testbed.  Following our protocol, we will be estimating the wireless
